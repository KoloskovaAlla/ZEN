...
const Slider = () => {
  ...
  useEffect(() => {
    ...
    dispatch(setSliderDescription(sliderDescription));

    slidesRef.current.style.width = `${width * slides.length}px`;
    slidesRef.current.style.left = `-${width * (currentSlideId - 1)}px`;
  }, [previewDetails, resize, currentSlideId, slides, setSliderDescription]);
  ...
  return (
    <div onClick={handleSliderClick} ref={sliderRef} className={classes.slider}>
      ...
      <button
        disabled={currentSlideId === slides.length}
      ...
      >{`${currentSlideId} / ${slides.length}`}</p>
    </div>
  );
};
